...
const [solutionPath, name, testSource] = process.argv.slice(2);
...
const readRemoteTest = async (url) => {
    const remoteUrl = url.replace(
        /^https:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/([^/]+)\/(.*)$/,
        "https://raw.githubusercontent.com/$1/$2/$3/$4",
    );
    const response = await globalThis._fetch(remoteUrl);
    if (!response.ok) {
        fatal(`Missing test for ${name}`);
    }
    const test = await response.text();
    return {
        test,
        mode: modes[extname(new URL(remoteUrl).pathname)],
    };
};
        ...
        if (err?.code === "ERR_MODULE_NOT_FOUND") {
            fatal(
                "Missing dependency: puppeteer.\n" +
                    "Install it or run a non-DOM exercise.",
            );
        }
        throw err;
    }
        ...
        (server = http.createServer(({ url, method }, response) => {
            ...
            try {
                ...
                deepStrictEqual.$ = async (selector, props) => {
                    ...
                    const domProps = await page.$eval(
                        selector,
                        extractProps,
                        props,
                    );
                ...
                deepStrictEqual.css = async (selector, props) => {
                    const cssProps = await page.evaluate(
                        (selector, props) => {
                            ...
                            if (!styles[selector]) {
                                throw Error(
                                    `css ${selector} did not match any declarations`,
                                );
                            }
                ...
                browser
                    ...
                    .overridePermissions(`http://localhost:${PORT}`, [
                        "clipboard-read",
                    ]);
...
const main = async () => {
    ...
    const remoteTests = testSource ? [readRemoteTest(testSource)] : [];
    const { test, mode } = await any([...localTests, ...remoteTests]).catch(
        ifNoEnt(() => fatal(`Missing test for ${name}`)),
    );
